...
:allow-uri-read:
:show-link-uri:
:orig: Mein custom attribute für Originalsprache/ Aussagen - HTML-Definition am Ende des Dokuments
:stylesdir: ./
//:stylesheet: /home/dennis/owncloud_maw/Mag/kollektivistisch/compact.css
...
= Searle's Auffassung von Kollektivit&auml;t
:doctype: book
:preface-title: Documentation Preface
:sectnums:
:toc: right
...
*A2*: ``Äußerlich betrachtet sind die beiden Fälle^Regen und corps de ballet^ ununterscheidbar, aber innerlich sind sie offensichtlich verschieden.''footnote:[S. 102]

*A3*: Trotz des identischen Verhaltens kann im Falle des Regenbeispiels jedes Individuum seine Absicht ohne Bezugnahme auf die anderen ausgedrückt werden.footnote:[S. 103]

*A4*: Die Summierung der individuellen Absichten führt nicht zur kollektiven Absicht.
  .. Niemand kann ein Passspiel alleine durchführen. So muss schon der Werfer eine andere individuelle Absicht haben als der Fänger.
  .. Die individuelle Absicht wird von der kollektiven Absicht abgeleitet.
...
Die Spieler der Softballmannschaft sprechen sich darin ab, beim Suchen jeder für sich zu handeln. Seinen Teil des Handelns ergibt sich aus der gemeinsamen Entscheidung beim Suchen nicht miteinander zu kooperieren.
  ...
  .. [orig]#Die Absprache ist wichtiger als das Verhalten selbst.# (worauf lässt sich begrifflich _Absprache_ und _Verhalten_ zurück führen?). Rückführung auf Intentionalität: Absprache ist der ursprüngliche Moment, auf dem sich die relevante Einstellung der Gruppenmitglieder. Das Verhalten ist jedoch auch auf die Absprache zurück zu führen. Ist für die Bestimmung von `kollektiv' aber nicht definierend. 
...
* *Analytisches Problem* ``seinen Teil tun''^Zitat von TuoMi^:  Die Interpretation von _seinen Teil tun_ führt zu einer zirkulären Rekursion.
* *Inadäquatheit* Die Formulierung lässt eine nicht-kooperative Variante zulässt. D.h.Der Teil der gemäß der Adam-Smith-Theorie dem handelnden Subjekt zugeordnet wurde, erfordert was keine Involvierung der Abstimmung unter den Individuen.


== Ziel: Wechselseitig geteilte Ueberzeugung ohne Kooperation ist kein Kollektiv
Das Gegenteil von kooperativer Handlung der Gruppe ist: aufgrund von geteilten, d.h. identischer Intentionalität (Überzeugung, Absicht et al) das Gleiche tun.
* Geteilte Überzeugung heißt... (kongruente Intentionalität unter den Individuen)
